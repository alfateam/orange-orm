function act(c) {
	...
	c.expected2 = "select order_0.lId as sorder_00,order_0.lLineNo as sorder_01,order_0.lOrderId as sorder_02 from orderLine order_0 INNER JOIN (select * from order order where order.oOrderId=2 AND order.discriminatorColumn='foo' AND order.discriminatorColumn2='baz' order by order.oCustomerId desc limit 8) order ON (order_0.lOrderId=order.oOrderId AND order.discriminatorColumn='foo' AND order.discriminatorColumn2='baz') where order.oOrderId=2 order by order_0.lId";
