...
function applyPatch(options = {}, dto, changes) {
    ...
    validateConflict(options.concurrency, dtoCompare, changes);
    rfc.applyPatch(dtoCompare, changes);
    let result = fromCompareObject(dtoCompare, options.schema, options.concurrency);
    ...
    function validateConflict(concurrency = {}, object, changes) {
        const set = [];
    ...
    function fromCompareObject(object, schema = {}) {
        if (Array.isArray(schema)) {
            let copy = [];

            let i = 0;
            for (let id in object) {
                copy[i] = fromCompareObject(object[id], schema[0]);
                i++;
            }
            return copy;
            ...
            for (let name in object) {
                if (name in schema || !hasSchema)
                    copy[name] = fromCompareObject(object[name], schema[name]);
            }
            return copy;
        }
        return object
