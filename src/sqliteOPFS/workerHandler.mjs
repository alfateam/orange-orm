// Shared protocol implementation for the bundled and explicit-module workers.
// Keep database state, leases and the operation queue local to each worker.
export default function installSqliteWorker(initializeSqlite) {
	let sqlite3Promise;
	let db;
	let dbOpenOptions;
	let dbOpenInfo;
	...
	async function getSqlite3() {
		if (!sqlite3Promise)
			sqlite3Promise = initializeSqlite(dbOpenOptions);
